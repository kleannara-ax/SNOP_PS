...
FILL_COND   = PatternFill('solid', fgColor='DAEEF3')       # 연한 청록 (조건부)
...
m_rules = [
    ...
    ["'D' 일별", 'out_sales_adj', '조건부: 수정이력 있으면 유지, 없으면 SAP값', '_modified=Y & 이번달 → 유지 / 그 외 → ETC_SALES 누적값'],
    ["'D' 일별", 'out_mill_roll_adj', '조건부: 수정이력 있으면 유지, 없으면 SAP값', '_modified=Y & 이번달 → 유지 / 그 외 → ETC_MILL_ROLL 누적값'],
    ["'D' 일별", 'out_etc_adj', '항상 SAP값으로 리셋', 'ETC_OUT 누적값으로 덮어씌움'],
    ["'D' 일별", 'out_disposal', '항상 사용자값 유지', 'SAP 무관 사용자 판단값 — 전일 확정값 유지'],
...
r += 1
style_cell(ws7, r, 2, '', font=FONT_NORMAL, fill=FILL_COND, alignment=ALIGN_CENTER)
style_cell(ws7, r, 3, '조건부 유지/SAP값 — 사용자 수정 이력 있으면(이번 달) 유지, 없으면 SAP값 세팅, 월 변경 시 리셋',
           font=FONT_SMALL, fill=FILL_COND, alignment=ALIGN_LEFT, merge_end_col=7, border=Border())
r += 1
style_cell(ws7, r, 2, '', font=FONT_NORMAL, fill=FILL_RESET, alignment=ALIGN_CENTER)
style_cell(ws7, r, 3, '항상 SAP값 리셋 — 사용자 수정 여부와 무관하게 매일 SAP값으로 덮어씌움',
...
style_cell(ws7, r, 3, '항상 사용자값 유지 — SAP와 무관하게 사용자 입력값 그대로 유지',
...
# ── 저장 ──
output_path = '/home/user/webapp/docs/RFC_설계서_Z_SNOP_PS_OBSOLETE_INV_GET_v1.2.xlsx'
import os
