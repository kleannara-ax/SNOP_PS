...
--   - 사용자 저장 버튼 클릭:
--       · 수정 4개 필드 UPDATE
--       · 매출(수정), 밀롤(수정) 값이 변경되었으면 _modified = 'Y'
...
CREATE TABLE IF NOT EXISTS ps_obsolete_inventory (
    ...
    out_sales_adj       DECIMAL(15,2)   DEFAULT 0.00             COMMENT '매출(수정) - 사용자 수정 후 저장 시 값 유지, 미수정 시 SAP값 세팅',
    out_sales_adj_modified CHAR(1)      DEFAULT 'N'              COMMENT '매출(수정) 사용자 수정 여부 (Y:수정됨→값유지, N:미수정→SAP값 사용)',
    out_mill_roll_adj   DECIMAL(15,2)   DEFAULT 0.00             COMMENT '밀롤(수정) - 사용자 수정 후 저장 시 값 유지, 미수정 시 SAP값 세팅',
    out_mill_roll_adj_modified CHAR(1)  DEFAULT 'N'              COMMENT '밀롤(수정) 사용자 수정 여부 (Y:수정됨→값유지, N:미수정→SAP값 사용)',
    out_disposal        DECIMAL(15,2)   DEFAULT 0.00             COMMENT '폐기 - 항상 사용자 입력값 유지 (SAP 무관)',
    out_etc_adj         DECIMAL(15,2)   DEFAULT 0.00             COMMENT '기타출고(수정) - 항상 SAP값으로 리셋, 사용자 수정 가능',
...
-- ============================================================
-- ★ 필드별 리셋 정책:
--   · 매출(수정)   → _modified='Y' & 이번 달 → 사용자 값 유지 / 그 외 → SAP값 세팅
--   · 밀롤(수정)   → _modified='Y' & 이번 달 → 사용자 값 유지 / 그 외 → SAP값 세팅
--   · 기타출고(수정) → 항상 SAP 기타출고 값으로 리셋
--   · 폐기          → 항상 사용자 입력값 유지 (SAP 무관)
-- ★ 월 변경 시: _modified 플래그 자동 리셋 (confirmed_dt 월 비교)
-- ★ ref_date → 당일로 갱신
...
--
--     -- ★ 매출(수정): 사용자가 수정 저장한 적 있고 이번 달이면 유지, 아니면 SAP값
--     out_sales_adj     = CASE
--                           WHEN out_sales_adj_modified = 'Y'
--                                AND DATE_FORMAT(confirmed_dt, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m')
--                           THEN out_sales_adj
--                           ELSE VALUES(out_sales_adj)
--                         END,
--     out_sales_adj_modified = CASE
--                           WHEN out_sales_adj_modified = 'Y'
--                                AND DATE_FORMAT(confirmed_dt, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m')
--                           THEN 'Y'
--                           ELSE 'N'
...
--
--     -- ★ 밀롤(수정): 동일 로직
--     out_mill_roll_adj = CASE
--                           WHEN out_mill_roll_adj_modified = 'Y'
--                                AND DATE_FORMAT(confirmed_dt, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m')
--                           THEN out_mill_roll_adj
--                           ELSE VALUES(out_mill_roll_adj)
--                         END,
--     out_mill_roll_adj_modified = CASE
--                           WHEN out_mill_roll_adj_modified = 'Y'
--                                AND DATE_FORMAT(confirmed_dt, '%Y-%m') = DATE_FORMAT(CURDATE(), '%Y-%m')
--                           THEN 'Y'
--                           ELSE 'N'
--                         END,
--
--     -- ★ 기타출고(수정): 항상 SAP값으로 리셋
--     out_etc_adj       = VALUES(out_etc_adj),
--
--     -- ★ 폐기: 항상 사용자 입력값 유지 (UPDATE 하지 않음)
...
-- → 수정 가능한 4개 필드 UPDATE + 확정 처리 + _modified 플래그 세팅
-- ============================================================
-- UPDATE ps_obsolete_inventory
-- SET
--     out_sales_adj     = #{outSalesAdj},
--     out_sales_adj_modified = 'Y',               -- 사용자가 매출(수정) 저장함
--     out_mill_roll_adj = #{outMillRollAdj},
--     out_mill_roll_adj_modified = 'Y',            -- 사용자가 밀롤(수정) 저장함
